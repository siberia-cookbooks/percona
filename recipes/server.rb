...
execute "assign-root-password-socket" do
  command "/opt/local/bin/mysqladmin --no-defaults -u root password \"#{node['mysql']['server_root_password']}\""
  action :run
  only_if "/opt/local/bin/mysql --no-defaults -u root -e 'show databases;' "
end

execute "assign-root-password-localhost" do
  command "/opt/local/bin/mysql -u root mysql -e \"SET PASSWORD FOR 'root'@'127.0.0.1' = PASSWORD('#{node['mysql']['server_root_password']}');\""
  Chef::Log.info("/opt/local/bin/mysql -u root mysql -e \"SET PASSWORD FOR 'root'@'127.0.0.1' = PASSWORD('#{node['mysql']['server_root_password']}');\"")
  only_if "/opt/local/bin/mysql --no-defaults -u root -e 'show databases;' "
  action :run
end
